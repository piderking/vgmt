...
class DexcomWebWorker(Thread):
    ...
    def fcn(self, index: int, data: dict):
        if data["type"] == "month":
            ...
            return [self.requestData(data["url"], data["year"],data["month"], asList=True)]
        elif data["type"] == "day":
            # Fetch Day Data (in-thread configuration)
            return [self.requestDayData(data["url"], data["year"],data["month"], data["day"], asList=True)]


        raise Warning("Type of Data to fetch isn't specified")
